...
void QModModule::cycleMode() {
    mode = (mode + 1) % NUM_MODES;
    for (int i = 0; i < NUM_SLOTS; i++) slotMode[i] = mode;
}
...
void QModModule::setGlobalMode(int newMode) {
    mode = math::clamp(newMode, 0, (int)NUM_MODES - 1);
    for (int i = 0; i < NUM_SLOTS; i++) slotMode[i] = mode;
}
...
float QModModule::frequencyHz(int slot) const {
    float base = (rateOverride >= 0.f) ? rateOverride : globalRate;
    if (!rateSpread || NUM_SLOTS <= 1) return base;
    // Log-spaced from base (slot 0, fastest) to base * spreadRatio (slot
    // N-1). Smaller spreadRatio digs deeper into the slow end.
    float r = math::clamp(spreadRatio, 1e-4f, 1.f);
    float t = (float)slot / (float)(NUM_SLOTS - 1);
    return base * std::pow(r, t);
}
        ...
        switch (inputMode) {
            ...
            case INPUT_CV_MODE: {
                int newMode = (int)std::min((float)(NUM_MODES - 1),
                                            t * (float)NUM_MODES);
                if (newMode != mode) setGlobalMode(newMode);
                break;
            }
        }
    }
...
void QModModule::dataFromJson(json_t* root) {
    ...
    // Default every slot to the (possibly newly-loaded) global mode first,
    // so legacy saves with no slotModes array still produce sensible state.
    for (int i = 0; i < NUM_SLOTS; i++) slotMode[i] = mode;
...
// Master mode-cycle button. Left-click steps through the 4 modes; each mode
// shows its own colour on the LED core.
struct ModeCycleButton : widget::OpaqueWidget {
    ...
    void drawLayer(const DrawArgs& args, int layer) override {
        if (layer == 1 && qm) {
            drawCenterDotGlow(args.vg, box.size.x, box.size.y, modeColor(qm->mode), 0.7f, dotScale);
        }
        Widget::drawLayer(args, layer);
    }
    ...
    void onButton(const event::Button& e) override {
        ...
        if (e.action == GLFW_PRESS && e.button == GLFW_MOUSE_BUTTON_LEFT && qm) {
            json_t* before = qm->dataToJson();
            qm->cycleMode();
            pushQModJsonAction(qm, before, "qmod mode");
            e.consume(this);
        }
    }
};
...
struct QModOutputPort : ThemedPJ301MPort {
    ...
    void draw(const DrawArgs& args) override {
        ...
        if (module->leftExpander.module
            && module->leftExpander.module->model == modelQMap) linked = true;
        else if (module->rightExpander.module
            && module->rightExpander.module->model == modelQMap) linked = true;
        if (!linked) return;
...
QModWidget::QModWidget(QModModule* module) {
    ...
    // Header row: mode-cycle button at slot 1's x, trigger input at slot 2's
    // x. Both centred on y = 14.5 mm.
    ...
    }
    {
        addInput(createInputCentered<lc::WhiteRingPJ301MPort>(
            mm2px(Vec(colR_mm, headerY)), module, QModModule::IN_TRIG));
    }
...
void QModWidget::appendContextMenu(Menu* menu) {
    ...
    menu->addChild(new MenuSeparator);
    menu->addChild(createMenuLabel("Mod mode"));
    // Menu items broadcast (like clicking the master button) so per-slot
    // overrides get reset to whichever mode is picked.
    menu->addChild(createCheckMenuItem("Random triggers", "",
        [=]() { return qm->mode == QModModule::MODE_RAND_TRIG; },
        [=]() { qm->setGlobalMode(QModModule::MODE_RAND_TRIG); }));
    menu->addChild(createCheckMenuItem("Triggered S+H (ext. clock)", "",
        [=]() { return qm->mode == QModModule::MODE_TRIG_SH; },
        [=]() { qm->setGlobalMode(QModModule::MODE_TRIG_SH); }));
    menu->addChild(createCheckMenuItem("Smooth random", "",
        [=]() { return qm->mode == QModModule::MODE_SMOOTH; },
        [=]() { qm->setGlobalMode(QModModule::MODE_SMOOTH); }));
    menu->addChild(createCheckMenuItem("Sample & hold", "",
        [=]() { return qm->mode == QModModule::MODE_SH; },
        [=]() { qm->setGlobalMode(QModModule::MODE_SH); }));
    menu->addChild(createCheckMenuItem("LFO", "",
        [=]() { return qm->mode == QModModule::MODE_LFO; },
        [=]() { qm->setGlobalMode(QModModule::MODE_LFO); }));
    ...
    auto inputModeLabel = [=]() -> std::string {
        switch (qm->inputMode) {
            case QModModule::INPUT_TRIGGER:       return "Trigger / resync";
            case QModModule::INPUT_GATE:          return "Gate (run/freeze)";
            case QModModule::INPUT_CV_RATE:       return "CV → rate";
            case QModModule::INPUT_CV_SMOOTHNESS: return "CV → smoothness";
            case QModModule::INPUT_CV_MODE:       return "CV → mode";
        }
        return "";
    };
    menu->addChild(createSubmenuItem("Input jack", inputModeLabel(),
        [=](ui::Menu* sub) {
            ...
            addInput("Trigger / resync",   QModModule::INPUT_TRIGGER);
            addInput("Gate (run/freeze)",  QModModule::INPUT_GATE);
            addInput("CV → rate",          QModModule::INPUT_CV_RATE);
            addInput("CV → smoothness",    QModModule::INPUT_CV_SMOOTHNESS);
            addInput("CV → mode",          QModModule::INPUT_CV_MODE);
        }));
